...
describes what users get. Pick a format with `-o`, or let lab choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
lab <command> -o table   # aligned columns for reading
lab <command> -o jsonl   # one JSON object per line, for piping
lab <command> -o json    # a single JSON array
lab <command> -o csv     # spreadsheet friendly
lab <command> -o tsv     # tab-separated
lab <command> -o url     # just the URL column
lab <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
lab <command> --fields id,title,url
```
...
```bash
lab <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
lab <command>            # a table, because this is a terminal
lab <command> | wc -l    # JSONL, because this is a pipe
```
